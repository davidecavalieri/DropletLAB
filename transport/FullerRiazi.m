...
function [Dij_HP] = FullerRiazi(Species,p, T, Xi,rho,rho0,mu,mu0)

%%%%%%%%%%%%%%%%%%%%%%%%%%% DESCRIPTION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FullerRiazi compute the binary mass diffusion coefficients using the
% Fuller-Riazi-Whitson method.
...
% 3) T                   : Temperature [K]
% 4) Xi                  : Mole fractions [-]
% 5) rho                 : Real-Fluid Mixture density [kg/m^3]
% 6) rho0                : Ideal-Fluid Mixture density [kg/m^3]
% 7) mu                  : Real-Fluid Mixture viscosity [Pa*s]
% 7) mu0                 : Ideal-Fluid Mixture viscosity [Pa*s]

% Output:
% Dij_HP                 : Matrix of the binary mass diffusion coefficients [m^2/s]
...
[b, c, Pr] = riazi_term(Species,Xi,p);

espRiazi  = b + c* Pr;
termIdeal = rho0*Dij;
termReal  = 1.07*((mu/mu0)^espRiazi);
Dij_HP    = (termReal*termIdeal)/(rho); % High-pressure diffusivity